...
const DEMO: Idea[] = [
  {
    id: 'did-1', org_id: 'demo', idea_name: 'Add online appointment booking widget',
    submitted_by: null, function: 'Patient Access & Front Desk',
    status: 'Under Review', date_added: '2025-04-20',
    description: 'Patients could self-schedule routine visits through a widget on the practice website, reducing front desk call volume.',
    linked_project: null, created_at: new Date().toISOString(),
  },
  {
    id: 'did-2', org_id: 'demo', idea_name: 'Weekly billing performance scorecard',
    submitted_by: null, function: 'Revenue Cycle & Finance',
    status: 'Approved', date_added: '2025-04-10',
    description: 'A one-page weekly report showing denial rate, claims submitted, and collection rate — shared with Dr. Evans and Markel every Monday.',
    linked_project: null, created_at: new Date().toISOString(),
  },
  {
    id: 'did-3', org_id: 'demo', idea_name: 'Patient re-engagement outreach for lapsed patients',
    submitted_by: null, function: 'Marketing',
    status: 'Parked', date_added: '2025-03-28',
    description: 'Identify patients who haven\'t been seen in over 12 months and send a re-engagement email or text.',
    linked_project: null, created_at: new Date().toISOString(),
  },
  {
    id: 'did-4', org_id: 'demo', idea_name: 'MA cross-training for front desk coverage',
    submitted_by: null, function: 'Human Resources & Staffing',
    status: 'Parked', date_added: '2025-04-15',
    description: 'Train MAs on basic front desk duties so they can provide coverage during absences.',
    linked_project: null, created_at: new Date().toISOString(),
  },
]
...
// ─── New Idea Modal ───────────────────────────────────────────────────────────
function NewIdeaModal({ orgId, onClose, onCreated }: {
  orgId: string; onClose: () => void; onCreated: () => void
}) {
  const supabase = createClient()
  const today = new Date().toISOString().split('T')[0]
  const [form, setForm] = useState({
    idea_name: '', function: '', description: '', date_added: today,
  })
      ...
      <div className="bg-[#1e1409] border border-[#2e2016] rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#2e2016]">
          <h2 className="text-[#c4b49a] font-semibold flex items-center gap-2">
            <Lightbulb size={15} className="text-[#facc15]" />Submit Idea
          </h2>
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div>
            ...
            <input value={form.idea_name} onChange={e => set('idea_name', e.target.value)}
...
export default function IdeasPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter(x =>
        x.idea_name.toLowerCase().includes(q) ||
  ...
  function exportCsv() {
    const headers = ['Idea', 'Status', 'Function', 'Date Added', 'Submitted By']
  ...
  return (
    <div className="flex flex-col h-full bg-[#1a1410] text-[#c4b49a]">
      ...
      <div className="px-6 pt-5 pb-3 border-b border-[#2e2016]">
        <h1 className="text-xl font-semibold text-[#c4b49a] flex items-center gap-2">
          <Lightbulb size={18} className="text-[#facc15]" />Ideas
        </h1>
        <p className="text-xs text-[#6b5a47] mt-0.5">
          {total} idea{total !== 1 ? 's' : ''} &nbsp;·&nbsp;
          <span className="text-[#86efac]">{approved} approved</span>
          &nbsp;·&nbsp;
          <span className="text-[#6b5a47]">{parked} parked</span>
          {!isManager && <span className="ml-2 text-[#c8843a]">· You can submit ideas — managers review &amp; approve</span>}
          ...
          <table className="w-full border-collapse text-xs min-w-[800px]">
            <thead>
              <tr className="bg-[#1e1409] border-b border-[#2e2016] sticky top-0 z-10">
                ...
                {[
                  { key: 'idea_name' as keyof Idea,   label: 'Idea',          icon: <Lightbulb size={11} />, w: 'min-w-[260px]' },
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={7} className="text-center py-16 text-[#6b5a47]">
                  <Loader2 size={20} className="animate-spin inline mr-2" />Loading ideas…
                </td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-16 text-[#6b5a47]">No ideas found. Be the first to submit one!</td></tr>
                ...
                return (
                  <tr key={row.id}
                    ...
                    {/* Status — managers can double-click to change */}
                    <td className={`px-3 ${rowPy}`}
                      onDoubleClick={e => {
                        e.stopPropagation()
                        if (isManager) setEditStatusId(row.id)
                      }}>
                      {editStatusId === row.id && isManager ? (
                        <SelectCell value={row.status} options={STATUS_OPTIONS}
                          onSave={v => saveStatus(row.id, v)} onCancel={() => setEditStatusId(null)} />
                      ) : (
                        <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${STATUS_COLORS[row.status] ?? ''}`}>
                          {row.status}
                          {!isManager && row.status === 'Parked' && <span className="ml-1 opacity-60">(pending review)</span>}
                        </span>
                      )}
                    </td>
                    ...
                    <td className={`px-3 ${rowPy} text-[#a08060] whitespace-nowrap`}>
                      {fmtDate(row.date_added)}
                    </td>

                    <td className={`px-3 ${rowPy} text-[#6b5a47] max-w-[250px] truncate`}>
                      {row.description ?? '—'}
                    </td>
                  </tr>
                )
              ...
              <tfoot>
                <tr className="border-t-2 border-[#2e2016] bg-[#1e1409] sticky bottom-0 text-[#6b5a47]">
                  ...
                  <td className="px-3 py-2">
                    <span className="text-[#86efac] text-[10px]">{approved} approved</span>
                    {' · '}
                    <span className="text-[#6b5a47] text-[10px]">{parked} parked</span>
                  </td>
                  <td colSpan={4} />
                </tr>
              </tfoot>
          ...
          <div className="w-[380px] border-l border-[#2e2016] bg-[#1e1409] flex flex-col overflow-y-auto shrink-0">
                ...
                <div className="border-t border-[#2e2016] pt-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {STATUS_OPTIONS.map(s => (
                      <button key={s}
                        onClick={() => saveStatus(detailRow.id, s)}
            ...
            {orgId && (
              <div className="border-t border-[#2e2016] px-4 py-3">
                <RecordComments recordId={detailRow.id} tableName="ideas" orgId={orgId} />
              </div>
            )}
      ...
      <div className="px-4 py-1.5 border-t border-[#2e2016] bg-[#1a1410]">
        <p className="text-[10px] text-[#6b5a47]">
          Anyone can submit ideas · Click row to view · {isManager ? 'Double-click Status or use panel buttons to approve/reject' : 'Managers review and approve ideas'}
        </p>
      </div>
    </div>
  )
