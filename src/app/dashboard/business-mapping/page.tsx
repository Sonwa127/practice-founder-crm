...
type FieldDef = ScalarField | SelectField | RelationField

const PROCESS_STATUSES = ['Not Started', 'Started', 'Active', 'To Improve', 'Excellent', 'Inactive']

const FORM_FIELDS: Record<TabKey, FieldDef[]> = {
  core_functions: [
    { key: 'core_function_name', label: 'Core Function Name', type: 'text',     required: true, placeholder: 'e.g. Clinical Care' },
  ...
  sops: [
    ...
    { key: 'how_we_know_complete', label: "How We Know It's Complete", type: 'textarea', placeholder: 'Success criteria — how do we confirm this SOP was executed correctly?' },
...
// ─── CRUD Modal ───────────────────────────────────────────────────────────────
function CrudModal({ tab, mode, initial, orgId, onClose, onSaved }: {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (mode === 'create') {
        const { error: err } = await supabase.from(tableName).insert({ ...payload, org_id: orgId })
        if (err) throw err
      } else {
        const { error: err } = await supabase.from(tableName).update(payload).eq('id', initial.id)
        if (err) throw err
      }
      onSaved(); onClose()
    } catch (e: unknown) {
            ...
            <div key={field.key}>
              ...
              {field.type === 'relation' && (
                <select className={inputCls + ' cursor-pointer'}
                  ...
                  {(relOptions[field.key] ?? []).map(o => (
                    <option key={o.id} value={o.id}>{o.label}</option>
                  ))}
                </select>
              )}
              {(field.type === 'text' || field.type === 'email' || field.type === 'tel' || field.type === 'number') && (
                <input className={inputCls} type={field.type}
                  placeholder={field.placeholder} value={String(form[field.key] ?? '')}
                  onChange={e => set(field.key, e.target.value)} />
              )}
            </div>
      ...
      <div className="bg-[#1e1409] border border-[#2e2016] rounded-xl shadow-2xl p-5 w-full max-w-sm mx-4">
        <div className="flex gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#6b5a47] mt-1"><span className="text-[#a08060]">"{name}"</span> will be permanently removed.</p>
          </div>
        </div>
...
function BizTable({ tab, orgId }: { tab: TabKey; orgId: string }) {
  ...
  return (
    <div className="flex flex-col h-full">
                  ...
                  <tr key={String(row.id ?? idx)}
                    ...
                    <td className="px-2 py-2.5 text-right">
                      <div className="inline-flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button onClick={e => openDelete(row, e)} title="Delete"
        ...
        {detailRow && (
          <div className="w-[340px] border-l border-[#2e2016] bg-[#1e1409] flex flex-col overflow-y-auto shrink-0">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#2e2016] sticky top-0 bg-[#1e1409] z-10">
              <h4 className="text-[#c4b49a] font-semibold text-sm truncate pr-2">
                {String(detailRow[cols[0].key] ?? 'Record')}
              </h4>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={e => openEdit(detailRow, e)} title="Edit"
                  className="p-1 rounded text-[#6b5a47] hover:text-[#c8843a] hover:bg-[#2e2016] transition-colors">
                  <Pencil size={13} />
                </button>
      ...
      {modal.open && (
        <CrudModal tab={tab} mode={modal.mode} initial={modal.row} orgId={orgId}
          onClose={() => setModal(m => ({ ...m, open: false }))}
          onSaved={() => { refetch(); if (modal.mode === 'edit' && detailRow?.id === modal.row.id) setDetailRow(null) }} />
      )}
      {deleteTarget && (
        <DeleteDialog name={String(deleteTarget[nameKey] ?? 'this record')}
