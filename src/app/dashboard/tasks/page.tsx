...
import {
  ...
  Calendar, LayoutGrid, Download, Upload, RefreshCw,
...
const DEMO_TASKS: Task[] = [
  { id: 'demo-1', org_id: 'demo', task_title: 'Review and update billing SOP', status: 'In-Process', due_date: '2025-05-10', assigned_to: null, function: 'Revenue Cycle & Finance', priority: 'Important & Not Urgent', type_of_task: 'Improve', frequency: 'One-Time', estimated_time: 2, actual_time: null, description: 'Review current billing SOP and update for new payer rules.', linked_project: null, linked_deliverable: null, linked_issue: null, submitted_by: null, created_at: new Date().toISOString() },
  { id: 'demo-2', org_id: 'demo', task_title: 'Complete daily chart closures', status: 'To-Do', due_date: '2025-05-01', assigned_to: null, function: 'Clinical Care', priority: 'Urgent & Important', type_of_task: 'Maintain-Routine', frequency: 'Daily', estimated_time: 1, actual_time: null, description: "Close all open charts from today's visits.", linked_project: null, linked_deliverable: null, linked_issue: null, submitted_by: null, created_at: new Date().toISOString() },
  { id: 'demo-3', org_id: 'demo', task_title: 'Follow up on denied claims', status: 'Manager Review', due_date: '2025-05-05', assigned_to: null, function: 'Revenue Cycle & Finance', priority: 'Urgent & Not Important', type_of_task: 'React', frequency: 'Trigger-Based', estimated_time: 3, actual_time: null, description: 'Investigate and resubmit denied claims from last week.', linked_project: null, linked_deliverable: null, linked_issue: null, submitted_by: null, created_at: new Date().toISOString() },
]
...
function NewTaskModal({ orgId, onClose, onCreated }: { orgId: string; onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    ...
    supabase.from('projects').select('id, project_name').eq('org_id', orgId)
      .not('status', 'in', '("Complete","Cancelled")').order('project_name')
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from('tasks').insert({
      ...
      linked_project: form.linked_project || null,
      ...
      <div className="bg-[#1e1409] border border-[#2e2016] rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        ...
        <div className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            ...
            <input value={form.task_title} onChange={e => set('task_title', e.target.value)} placeholder="Action-based title…"
          ...
          <div>
            <label className="block text-[#a08060] text-xs mb-1">Assigned To</label>
          ...
          <div>
            ...
            <input type="number" step="0.5" min="0" value={form.estimated_time} onChange={e => set('estimated_time', e.target.value)}
              placeholder="e.g. 1.5"
              className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none" />
          </div>
          <div>
            ...
            <textarea value={form.description} onChange={e => set('description', e.target.value)}
              placeholder="Purpose, resources needed, done criteria…" rows={3}
          ...
          <div>
            <label className="block text-[#a08060] text-xs mb-1">
              Linked Project <span className="text-[#6b5a47] font-normal">(optional)</span>
            </label>
          ...
          <div>
            <label className="block text-[#a08060] text-xs mb-1">
              Linked Issue <span className="text-[#6b5a47] font-normal">(optional — if this task resolves a known issue)</span>
            </label>
            <select value={form.linked_issue} onChange={e => set('linked_issue', e.target.value)}
              className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none">
              <option value="">— None —</option>
...
export default function TasksPage() {
  ...
  // Maps for resolving relation IDs in detail panel
  ...
  useEffect(() => {
    ...
    supabase.from('projects').select('id, project_name').eq('org_id', orgId)
      .then(({ data }) => {
        ...
        ;(data ?? []).forEach((r: { id: string; project_name: string }) => { m[r.id] = r.project_name })
        setProjectMap(m)
      })
    supabase.from('issues_breakdowns').select('id, issue_name').eq('org_id', orgId)
      .then(({ data }) => {
        const m: Record<string, string> = {}
        ;(data ?? []).forEach((r: { id: string; issue_name: string }) => { m[r.id] = r.issue_name })
        setIssueMap(m)
      })
  }, [orgId])
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter(x => x.task_title.toLowerCase().includes(q) || (x.function ?? '').toLowerCase().includes(q) || (x.status ?? '').toLowerCase().includes(q))
    }
  ...
  function exportCsv() {
    const headers = ['Title', 'Status', 'Due Date', 'Assigned To', 'Priority', 'Type', 'Function', 'Frequency', 'Est. Time', 'Actual Time']
    const csv = [headers, ...filtered.map(r => [
      r.task_title, r.status, r.due_date ?? '',
      r.assigned_to ? resolveName(r.assigned_to) : '',
      r.priority ?? '', r.type_of_task ?? '', r.function ?? '', r.frequency ?? '',
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-auto">
          <table className="w-full border-collapse text-xs min-w-[900px]">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={11} className="text-center py-16 text-[#6b5a47]"><Loader2 size={20} className="animate-spin inline mr-2" />Loading tasks…</td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={11} className="text-center py-16 text-[#6b5a47]">No tasks found.</td></tr>
              ) : filtered.map((row, idx) => {
                const selected = selectedIds.has(row.id)
                const overdueFlag = isOverdue(row.due_date, row.status)
                const isActive = detailRow?.id === row.id
                return (
                  <tr key={row.id} onClick={() => setDetailRow(isActive ? null : row)}
                    className={`border-b border-[#2e2016] cursor-pointer transition-colors group ${selected ? 'pf-row-selected' : ''} ${isActive ? 'bg-[#c8843a]/10' : idx % 2 === 0 ? 'bg-[#1a1410]' : 'bg-[#1c1610]'} hover:bg-[#c8843a]/5`}>
                    ...
                    <td className={`pf-sticky-cell left-8 px-3 ${rowPy} font-medium text-[#c4b49a]`} style={{ left: 32 }}
                    ...
                    <td className={`px-3 ${rowPy} whitespace-nowrap`}
                      ...
                      ) : <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${STATUS_COLORS[row.status] ?? 'bg-[#2e2016] text-[#c4b49a]'}`}>{row.status}</span>}
                    </td>
                    <td className={`px-3 ${rowPy} whitespace-nowrap ${overdueFlag ? 'text-[#f87171]' : 'text-[#c4b49a]'}`}
                    ...
                    <td className={`px-3 ${rowPy} text-[#a08060] text-right tabular-nums`}>{fmtTime(row.estimated_time)}</td>
                    <td className={`px-3 ${rowPy} text-[#a08060] text-right tabular-nums`}
                      ...
                      {editCell?.id === row.id && editCell?.key === 'actual_time' ? (
                        <input type="number" step="0.5" autoFocus defaultValue={row.actual_time ?? ''}
            ...
            {filtered.length > 0 && (
              <tfoot>
                <tr className="border-t-2 border-[#2e2016] bg-[#1e1409] text-[#6b5a47] font-medium sticky bottom-0">
                  ...
                  <td colSpan={5} />
                  <td className="px-3 py-2 text-right tabular-nums">{fmtTime(filtered.reduce((s, r) => s + (r.estimated_time ?? 0), 0))}</td>
                  <td className="px-3 py-2 text-right tabular-nums">{fmtTime(filtered.reduce((s, r) => s + (r.actual_time ?? 0), 0))}</td>
                </tr>
              </tfoot>
            )}
          </table>
        ...
        {detailRow && (
          <div className="w-[380px] border-l border-[#2e2016] bg-[#1e1409] flex flex-col overflow-y-auto shrink-0">
            ...
            <div className="px-4 py-3 space-y-3 text-xs">
              ...
              {(detailRow.linked_project || detailRow.linked_issue || detailRow.linked_deliverable) && (
                <div className="border-t border-[#2e2016] pt-3 space-y-1.5">
                  ...
                  {detailRow.linked_project && (
                    <p className="text-[#a08060] flex items-center gap-1.5"><ExternalLink size={10} />Project: <span className="text-[#c4b49a]">{projectMap[detailRow.linked_project] ?? detailRow.linked_project}</span></p>
                  )}
                  {detailRow.linked_issue && (
                    <p className="text-[#a08060] flex items-center gap-1.5"><ExternalLink size={10} />Issue: <span className="text-[#c4b49a]">{issueMap[detailRow.linked_issue] ?? detailRow.linked_issue}</span></p>
                  )}
                  {detailRow.linked_deliverable && (
                    <p className="text-[#a08060] flex items-center gap-1.5"><ExternalLink size={10} />Deliverable: <span className="text-[#c4b49a]">{detailRow.linked_deliverable}</span></p>
                  )}
                </div>
              )}
