...
type Issue = {
  id: string; org_id: string; issue_name: string
  submitted_by: string | null; function: string | null
  impact_level: string | null; status: string; priority: string | null
  description: string | null; root_cause: string | null
  link_to_huddle: string | null; linked_task: string | null
  created_at: string
}
...
const STATUS_COLORS: Record<string, string> = {
  'Open': 'bg-[#2e1010] text-[#f87171]', 'Investigating': 'bg-[#1a1a0f] text-[#facc15]',
  'Resolved': 'bg-[#0f2318] text-[#86efac]', 'Closed': 'bg-[#1e1409] text-[#6b5a47]',
}
const IMPACT_COLORS: Record<string, string> = {
  'Low': 'bg-[#1e1409] text-[#6b5a47]', 'Medium': 'bg-[#1a1a0f] text-[#facc15]',
  'High': 'bg-[#2e1a10] text-[#fb923c]', 'Critical': 'bg-[#2e1010] text-[#f87171]',
}
const PRIORITY_COLORS: Record<string, string> = {
  'Low': 'text-[#6b5a47]', 'Medium': 'text-[#facc15]', 'High': 'text-[#f87171]',
}

const DEMO: Issue[] = [
  { id: 'di-1', org_id: 'demo', issue_name: 'Claims not submitted within 24hrs', submitted_by: null, function: 'Revenue Cycle & Finance', impact_level: 'High', status: 'Investigating', priority: 'High', description: 'Several claims from Monday were not submitted within the required 24-hour window.', root_cause: 'Billing staff did not receive chart closure notification from Cerbo.', link_to_huddle: null, linked_task: null, created_at: new Date().toISOString() },
  { id: 'di-2', org_id: 'demo', issue_name: 'MA no-show — front desk coverage gap', submitted_by: null, function: 'Human Resources & Staffing', impact_level: 'Critical', status: 'Open', priority: 'High', description: 'MA called out with no backup available. Dr. Evans had to room patients herself.', root_cause: null, link_to_huddle: null, linked_task: null, created_at: new Date().toISOString() },
  { id: 'di-3', org_id: 'demo', issue_name: 'Insurance verification backlog', submitted_by: null, function: 'Patient Access & Front Desk', impact_level: 'Medium', status: 'Open', priority: 'Medium', description: "Upcoming week's patients have not had insurance verified.", root_cause: null, link_to_huddle: null, linked_task: null, created_at: new Date().toISOString() },
  { id: 'di-4', org_id: 'demo', issue_name: 'Duplicate billing code on AWV claims', submitted_by: null, function: 'Revenue Cycle & Finance', impact_level: 'Medium', status: 'Resolved', priority: 'Medium', description: 'AWV claims going out with a duplicate wellness code.', root_cause: 'Template misconfigured in OPM after system update.', link_to_huddle: null, linked_task: null, created_at: new Date().toISOString() },
]
...
// ─── New Issue Modal ──────────────────────────────────────────────────────────
function NewIssueModal({ orgId, onClose, onCreated }: { orgId: string; onClose: () => void; onCreated: () => void }) {
  const supabase = createClient()
  const [form, setForm] = useState({
    issue_name: '', status: 'Open', function: '',
    impact_level: 'Medium', priority: 'Medium',
    description: '', root_cause: '',
    link_to_huddle: '',   // ADDED
    linked_task: '',       // ADDED
  })
  const [saving, setSaving] = useState(false)
  const [err, setErr] = useState('')

  // ADDED: fetch huddle logs and tasks for relation dropdowns
  const [huddleLogs, setHuddleLogs] = useState<{ id: string; date: string }[]>([])
  const [tasks, setTasks] = useState<{ id: string; task_title: string }[]>([])
  useEffect(() => {
    supabase.from('daily_huddle_log').select('id, date').eq('org_id', orgId)
      .order('date', { ascending: false }).limit(30)
      .then(({ data }) => setHuddleLogs((data ?? []) as { id: string; date: string }[]))
    supabase.from('tasks').select('id, task_title').eq('org_id', orgId)
      .in('status', ['To-Do', 'In-Process']).order('task_title').limit(100)
      .then(({ data }) => setTasks((data ?? []) as { id: string; task_title: string }[]))
  }, [orgId])
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from('issues_breakdowns').insert({
      ...
      link_to_huddle: form.link_to_huddle || null,   // ADDED
      linked_task: form.linked_task || null,          // ADDED
    })
    setSaving(false)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-[#1e1409] border border-[#2e2016] rounded-xl w-full max-w-lg mx-4 shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#2e2016]">
          <h2 className="text-[#c4b49a] font-semibold flex items-center gap-2"><AlertTriangle size={15} className="text-[#fb923c]" />Log New Issue</h2>
        ...
        <div className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            ...
            <input value={form.issue_name} onChange={e => set('issue_name', e.target.value)} placeholder="Short description of what went wrong…"
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <select value={form.status} onChange={e => set('status', e.target.value)} className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none">
                {STATUS_OPTIONS.map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[#a08060] text-xs mb-1">Impact</label>
              <select value={form.impact_level} onChange={e => set('impact_level', e.target.value)} className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none">
                {IMPACT_OPTIONS.map(o => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-[#a08060] text-xs mb-1">Priority</label>
              <select value={form.priority} onChange={e => set('priority', e.target.value)} className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none">
          ...
          <div>
            ...
            <select value={form.function} onChange={e => set('function', e.target.value)} className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => set('description', e.target.value)} placeholder="Full description of the issue…" rows={3}
          ...
          <div>
            <label className="block text-[#a08060] text-xs mb-1">Root Cause <span className="text-[#6b5a47] font-normal">(optional — fill after investigation)</span></label>
            <textarea value={form.root_cause} onChange={e => set('root_cause', e.target.value)} placeholder="What caused this?" rows={2}
              className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none resize-none" />
          </div>
          {/* ADDED: Link to Daily Huddle Log */}
          <div>
            <label className="block text-[#a08060] text-xs mb-1">
              Link to Daily Huddle Log <span className="text-[#6b5a47] font-normal">(optional)</span>
            </label>
            <select value={form.link_to_huddle} onChange={e => set('link_to_huddle', e.target.value)}
              ...
              {huddleLogs.map(h => <option key={h.id} value={h.id}>{fmtDate(h.date)}</option>)}
            </select>
          </div>
          {/* ADDED: Linked Task */}
          <div>
            <label className="block text-[#a08060] text-xs mb-1">
              Linked Task <span className="text-[#6b5a47] font-normal">(optional — if a task was created to resolve this)</span>
            </label>
            <select value={form.linked_task} onChange={e => set('linked_task', e.target.value)}
              className="w-full bg-[#120d08] border border-[#2e2016] focus:border-[#c8843a] text-[#c4b49a] text-sm rounded px-3 py-2 outline-none">
              <option value="">— None —</option>
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-[#2e2016]">
          ...
          <button onClick={handleSubmit} disabled={saving}
            ...
            {saving ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />}Log Issue
          </button>
        </div>
      </div>
    </div>
...
// ─── Main page — identical to original except NewIssueModal receives orgId ────
export default function IssuesPage() {
  ...
  // ADDED: maps for resolving relation IDs in detail panel
  const [huddleMap, setHuddleMap] = useState<Record<string, string>>({})
  ...
  useEffect(() => {
    ...
    supabase.from('daily_huddle_log').select('id, date').eq('org_id', orgId)
      .then(({ data }) => {
        ...
        ;(data ?? []).forEach((r: { id: string; date: string }) => { m[r.id] = fmtDate(r.date) })
        setHuddleMap(m)
      })
    supabase.from('tasks').select('id, task_title').eq('org_id', orgId)
      .then(({ data }) => {
        const m: Record<string, string> = {}
        ;(data ?? []).forEach((r: { id: string; task_title: string }) => { m[r.id] = r.task_title })
        setTaskMap(m)
      })
  }, [orgId])
  ...
  const { data: issues, isLoading, refetch } = useQuery<Issue[]>({
    ...
    enabled: !!orgId,
    queryFn: async () => {
      const { data, error } = await supabase.from('issues_breakdowns').select('*').eq('org_id', orgId!).order('created_at', { ascending: false })
  ...
  const filtered = useMemo(() => {
    let r = rows
    if (search) { const q = search.toLowerCase(); r = r.filter(x => x.issue_name.toLowerCase().includes(q) || (x.function ?? '').toLowerCase().includes(q) || (x.status ?? '').toLowerCase().includes(q)) }
    ...
    if (sortConfig) {
      ...
      r = [...r].sort((a, b) => dir === 'asc' ? String(a[key] ?? '').localeCompare(String(b[key] ?? '')) : String(b[key] ?? '').localeCompare(String(a[key] ?? '')))
    }
    return r
  ...
  function exportCsv() {
    ...
    const csv = [headers, ...filtered.map(r => [r.issue_name, r.status, r.impact_level ?? '', r.priority ?? '', r.function ?? '', r.submitted_by ? resolveName(r.submitted_by) : '', fmtDate(r.created_at)])].map(r => r.map(c => `"${c}"`).join(',')).join('\n')
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv])); a.download = 'issues.csv'; a.click()
  }
    ...
    <div className="flex flex-col h-full bg-[#1a1410] text-[#c4b49a]">
      <div className="px-6 pt-5 pb-3 border-b border-[#2e2016]">
        ...
        <p className="text-xs text-[#6b5a47] mt-0.5">
          {total} issue{total !== 1 ? 's' : ''} &nbsp;·&nbsp; <span className="text-[#f87171]">{open} open</span>
      ...
      {showFilters && (
        <div className="px-4 py-2.5 border-b border-[#2e2016] bg-[#1e1409] flex items-center gap-3 flex-wrap">
          ...
          <select value={filterImpact} onChange={e => setFilterImpact(e.target.value)} className="bg-[#120d08] border border-[#2e2016] text-[#c4b49a] text-xs rounded px-2 py-1 outline-none focus:border-[#c8843a]">
            <option value="">All Impact Levels</option>{IMPACT_OPTIONS.map(o => <option key={o}>{o}</option>)}
          </select>
          ...
          {(filterStatus || filterImpact || filterFunction) && (
            <button onClick={() => { setFilterStatus(''); setFilterImpact(''); setFilterFunction('') }} className="flex items-center gap-1 text-xs text-[#f87171]"><X size={11} />Clear</button>
          )}
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 overflow-auto">
          <table className="w-full border-collapse text-xs min-w-[860px]">
            <thead>
              <tr className="bg-[#1e1409] border-b border-[#2e2016] sticky top-0 z-10">
                ...
                {[
                  ...
                  { key: 'link_to_huddle' as keyof Issue, label: 'Huddle Log', icon: <Link2 size={11} />, w: 'min-w-[110px]' }, // ADDED column
                  { key: 'linked_task' as keyof Issue,  label: 'Linked Task',  icon: <Link2 size={11} />, w: 'min-w-[180px]' }, // ADDED column
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={10} className="text-center py-16 text-[#6b5a47]"><Loader2 size={20} className="animate-spin inline mr-2" />Loading issues…</td></tr>
              ) : filtered.length === 0 ? (
                ...
                return (
                  <tr key={row.id} onClick={() => setDetailRow(isActive ? null : row)}
                    className={`border-b border-[#2e2016] cursor-pointer transition-colors group ${selected ? 'pf-row-selected' : ''} ${isActive ? 'bg-[#c8843a]/10' : isCritical ? 'bg-[#2e1010]/30' : idx % 2 === 0 ? 'bg-[#1a1410]' : 'bg-[#1c1610]'} hover:bg-[#c8843a]/5`}>
                    <td className="pf-sticky-checkbox px-2" onClick={e => e.stopPropagation()}>
                      <input type="checkbox" checked={selected}
                        onChange={e => { const next = new Set(selectedIds); e.target.checked ? next.add(row.id) : next.delete(row.id); setSelectedIds(next) }} className="accent-[#c8843a]" />
                    </td>
                    <td className={`pf-sticky-cell px-3 ${rowPy} font-medium text-[#c4b49a]`} style={{ left: 32 }}
                      ...
                      {editCell?.id === row.id && editCell?.key === 'issue_name' ? (
                        <TextCell value={row.issue_name} onSave={v => saveCell(row.id, 'issue_name', v)} onCancel={() => setEditCell(null)} />
                      ) : (
                        <span className="flex items-center gap-1.5">{isCritical && <AlertTriangle size={11} className="text-[#f87171] shrink-0" />}{row.issue_name}</span>
                      )}
                    </td>
                    ...
                    <td className={`px-3 ${rowPy}`} onDoubleClick={e => { e.stopPropagation(); setEditCell({ id: row.id, key: 'impact_level' }) }}>
                      {editCell?.id === row.id && editCell?.key === 'impact_level' ? (
                        <SelectCell value={row.impact_level ?? ''} options={IMPACT_OPTIONS} onSave={v => saveCell(row.id, 'impact_level', v)} onCancel={() => setEditCell(null)} />
                      ) : <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${IMPACT_COLORS[row.impact_level ?? ''] ?? 'bg-[#1e1409] text-[#6b5a47]'}`}>{row.impact_level ?? '—'}</span>}
                    </td>
                    <td className={`px-3 ${rowPy}`} onDoubleClick={e => { e.stopPropagation(); setEditCell({ id: row.id, key: 'priority' }) }}>
                      ...
                      ) : <span className={`text-xs font-medium ${PRIORITY_COLORS[row.priority ?? ''] ?? 'text-[#6b5a47]'}`}>{row.priority ?? '—'}</span>}
                    </td>
                    ...
                    {/* ADDED: huddle link column */}
                    <td className={`px-3 ${rowPy} text-[#a08060] whitespace-nowrap`}>
                      {row.link_to_huddle ? (
                        <span className="flex items-center gap-1 text-[#c8843a]"><Link2 size={10} />{huddleMap[row.link_to_huddle] ?? '…'}</span>
                      ) : <span className="text-[#6b5a47]">—</span>}
                    </td>
                    {/* ADDED: linked task column */}
                    <td className={`px-3 ${rowPy} text-[#6b5a47] max-w-[200px] truncate`}>
                      {row.linked_task ? (taskMap[row.linked_task] ?? '…') : '—'}
                    </td>
                    <td className={`px-3 ${rowPy} text-[#6b5a47] whitespace-nowrap`}>{fmtDate(row.created_at)}</td>
                  </tr>
                )
              })}
            </tbody>
            {filtered.length > 0 && (
              <tfoot>
                <tr className="border-t-2 border-[#2e2016] bg-[#1e1409] text-[#6b5a47] sticky bottom-0">
                  <td className="px-2 py-2" /><td className="px-3 py-2 text-[#a08060] font-medium">{filtered.length} issues</td>
                  <td className="px-3 py-2"><span className="text-[#f87171] text-[10px]">{open} open</span></td>
                  <td className="px-3 py-2">{critical > 0 && <span className="text-[#f87171] text-[10px] font-semibold">{critical} critical</span>}</td>
        ...
        {detailRow && (
          <div className="w-[380px] border-l border-[#2e2016] bg-[#1e1409] flex flex-col overflow-y-auto shrink-0">
            <div className="flex items-start justify-between px-4 py-3 border-b border-[#2e2016] sticky top-0 bg-[#1e1409] z-10">
              <div className="flex-1 pr-2">
                <div className="flex items-center gap-2 mb-1.5">
                  <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${STATUS_COLORS[detailRow.status] ?? ''}`}>{detailRow.status}</span>
                  {detailRow.impact_level && <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${IMPACT_COLORS[detailRow.impact_level]}`}>{detailRow.impact_level} Impact</span>}
                </div>
            ...
            <div className="px-4 py-3 space-y-3 text-xs">
              ...
              {/* ADDED: relation links in detail panel — now properly labelled */}
              {(detailRow.linked_task || detailRow.link_to_huddle) && (
                <div className="border-t border-[#2e2016] pt-3 space-y-1.5">
                  ...
                  {detailRow.link_to_huddle && (
                    <p className="text-[#a08060]">Huddle Log: <span className="text-[#c4b49a]">{huddleMap[detailRow.link_to_huddle] ?? detailRow.link_to_huddle}</span></p>
                  )}
                  {detailRow.linked_task && (
                    <p className="text-[#a08060]">Task: <span className="text-[#c4b49a]">{taskMap[detailRow.linked_task] ?? detailRow.linked_task}</span></p>
                  )}
                </div>
              )}
              ...
              <div className="border-t border-[#2e2016] pt-3">
                <p className="text-[#6b5a47] mb-1.5 font-medium flex items-center gap-1"><ShieldAlert size={11} />Root Cause</p>
              ...
              <div className="border-t border-[#2e2016] pt-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {STATUS_OPTIONS.map(s => (
                    <button key={s} onClick={() => saveCell(detailRow.id, 'status', s)}
                      className={`px-2.5 py-1 rounded-full text-[10px] font-medium transition-colors ${detailRow.status === s ? (STATUS_COLORS[s] ?? 'bg-[#c8843a] text-white') + ' ring-1 ring-white/20' : 'bg-[#120d08] text-[#6b5a47] hover:text-[#c4b49a] border border-[#2e2016]'}`}>
                      {s}
                    </button>
                  ))}
                </div>
            ...
            {orgId && <div className="border-t border-[#2e2016] px-4 py-3"><RecordComments recordId={detailRow.id} tableName="issues_breakdowns" orgId={orgId} /></div>}
      ...
      {showNewModal && orgId && <NewIssueModal orgId={orgId} onClose={() => setShowNewModal(false)} onCreated={() => queryClient.invalidateQueries({ queryKey: ['issues', orgId] })} />}
