...
function NewIssueModal({ orgId, onClose, onCreated }: { orgId: string; onClose: () => void; onCreated: () => void }) {
  ...
  const [employees,  setEmployees]  = useState<{ id: string; name: string }[]>([])   // ← ADDED
  ...
  async function handleSubmit() {
    ...
    const { error } = await supabase.from('issues_breakdowns').insert({
      ...
      link_to_huddle: form.link_to_huddle  || null,
      linked_task:    form.linked_task     || null,
      submitted_by:   form.submitted_by    || null,   // ← ADDED
    })
    setSaving(false)
        ...
        <div className="px-5 py-4 space-y-3 max-h-[75vh] overflow-y-auto">
          ...
          <div>
            <label className="block text-[#a08060] text-xs mb-1">
              Submitted By <span className="text-[#6b5a47] font-normal">(who is logging this issue?)</span>
            </label>
          ...
          <div>
            <label className="block text-[#a08060] text-xs mb-1">
              Root Cause <span className="text-[#6b5a47] font-normal">(optional — fill after investigation)</span>
            </label>
...
export default function IssuesPage() {
  ...
  // Maps for relation labels in detail panel
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      r = r.filter(x => x.issue_name.toLowerCase().includes(q) || (x.function ?? '').toLowerCase().includes(q) || x.status.toLowerCase().includes(q))
    }
  ...
  function exportCsv() {
    ...
    const csv = [headers, ...filtered.map(r => [
      r.issue_name, r.status, r.impact_level ?? '', r.priority ?? '', r.function ?? '',
      r.submitted_by ? resolveName(r.submitted_by) : '', fmtDate(r.created_at)
            ...
            <thead>
              <tr className="bg-[#1e1409] border-b border-[#2e2016] sticky top-0 z-10">
                ...
                {[
                  ...
                  { key: 'link_to_huddle'as keyof Issue, label: 'Huddle Log',   icon: <Link2 size={11} />,         w: 'min-w-[110px]' },
                  { key: 'linked_task'   as keyof Issue, label: 'Linked Task',  icon: <Link2 size={11} />,         w: 'min-w-[180px]' },
              ...
              <div className="flex-1 pr-2">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  {detailRow.impact_level && (
                    <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${IMPACT_COLORS[detailRow.impact_level]}`}>{detailRow.impact_level} Impact</span>
                  )}
                </div>
              ...
              {(detailRow.linked_task || detailRow.link_to_huddle) && (
                <div className="border-t border-[#2e2016] pt-3 space-y-1.5">
                  ...
                  {detailRow.link_to_huddle && (
                    <p className="text-[#a08060]">Huddle: <span className="text-[#c4b49a]">{huddleMap[detailRow.link_to_huddle] ?? detailRow.link_to_huddle}</span></p>
                  )}
                  ...
                  {STATUS_OPTIONS.map(s => (
                    <button key={s} onClick={() => saveCell(detailRow.id, 'status', s)}
                      className={`px-2.5 py-1 rounded-full text-[10px] font-medium transition-colors
                        ${detailRow.status === s ? (STATUS_COLORS[s] ?? 'bg-[#c8843a] text-white') + ' ring-1 ring-white/20' : 'bg-[#120d08] text-[#6b5a47] hover:text-[#c4b49a] border border-[#2e2016]'}`}>
                      {s}
                    </button>
                  ))}
                </div>
