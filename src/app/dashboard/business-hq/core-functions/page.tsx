...
function CoreFunctionsContent() {
  ...
  const emptyForm = { name: '', goal: '', how_it_is_done: '' }
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return data as CoreFunction[]
    },
  })

  const invalidate = () => qc.invalidateQueries({ queryKey: ['core-functions', orgId] })

  const createMutation = useMutation({
    mutationFn: async (p: typeof emptyForm) => {
      const { error } = await supabase.from('core_functions').insert({ org_id: orgId, name: p.name, goal: p.goal || null, how_it_is_done: p.how_it_is_done || null })
      if (error) throw error
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, field, value }: { id: string; field: string; value: string }) => {
      const { error } = await supabase.from('core_functions').update({ [field]: value || null }).eq('id', id)
      if (error) throw error
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (ids: string[]) => {
      const { error } = await supabase.from('core_functions').delete().in('id', ids)
      if (error) throw error
    },
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-[#c4b49a]">Core Functions</h1>
          <p className="text-sm text-[#c4b49a]/60 mt-1">The fundamental areas that define what this practice does</p>
        </div>
        <button onClick={() => setShowForm(true)} className="px-4 py-2 bg-[#c8843a] hover:bg-[#b8732a] text-white text-sm font-medium rounded-lg transition-colors">
          + New Function
        </button>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search functions…" className="w-full pl-9 pr-4 py-2 bg-[#1f1a14] border border-[#2e2016] rounded-lg text-sm text-[#c4b49a] placeholder-[#c4b49a]/30 focus:outline-none focus:border-[#c8843a]/50" />
        </div>
        {selectedIds.size > 0 && (
      ...
      <div className="bg-[#1f1a14] border border-[#2e2016] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#2e2016]">
              ...
              <th className="p-3 text-left text-xs uppercase tracking-widest text-[#c4b49a]/50 font-medium">Function Name</th>
              <th className="p-3 text-left text-xs uppercase tracking-widest text-[#c4b49a]/50 font-medium">Goal</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={4} className="p-12 text-center text-[#c4b49a]/40 text-sm">No core functions yet. Add the first one.</td></tr>
            ) : filtered.map(item => (
              <>
                <tr key={item.id} className={`border-b border-[#2e2016]/50 hover:bg-[#2a1f14]/40 transition-colors ${expandedId === item.id ? 'bg-[#2a1f14]/40' : ''}`}>
                  ...
                  <td className="p-3 text-[#c4b49a]/60 text-xs max-w-xs truncate">{item.goal ?? '—'}</td>
                </tr>
                    ...
                    <td colSpan={4} className="p-0">
                      <div className="p-6 grid grid-cols-2 gap-8">
                        <div className="space-y-5">
                          {[
                            { label: 'Function Name', field: 'name', value: item.name, multiline: false },
                            { label: 'Goal', field: 'goal', value: item.goal, multiline: true },
                            { label: 'How It Is Done', field: 'how_it_is_done', value: item.how_it_is_done, multiline: true },
                          ].map(f => (
                            <div key={f.field}>
                        ...
                        <div><RecordComments orgId={orgId} recordId={item.id} tableName="core_functions" /></div>
                      </div>
                      <div className="px-6 pb-4 flex justify-end">
                        <button onClick={() => setConfirmDelete([item.id])} className="text-xs text-red-400/60 hover:text-red-400 transition-colors">Delete function</button>
                      </div>
                    </td>
                  </tr>
                )}
        ...
        {filtered.length > 0 && <div className="px-4 py-3 border-t border-[#2e2016] text-xs text-[#c4b49a]/40">Showing {filtered.length} of {items.length} functions</div>}
      </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1f1a14] border border-[#2e2016] rounded-2xl w-full max-w-lg p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-[#c4b49a]">New Core Function</h2>
            ...
            <div>
              <label className="block text-xs text-[#c4b49a]/50 uppercase tracking-widest mb-1">Function Name *</label>
            ...
            <div>
              <label className="block text-xs text-[#c4b49a]/50 uppercase tracking-widest mb-1">Goal</label>
              <textarea value={form.goal} onChange={e => setForm(p => ({ ...p, goal: e.target.value }))} rows={2} className="w-full bg-[#1a1410] border border-[#2e2016] rounded-lg px-3 py-2 text-sm text-[#c4b49a] focus:outline-none focus:border-[#c8843a]/50 resize-none" />
            </div>
            <div>
              <label className="block text-xs text-[#c4b49a]/50 uppercase tracking-widest mb-1">How It Is Done</label>
              <textarea value={form.how_it_is_done} onChange={e => setForm(p => ({ ...p, how_it_is_done: e.target.value }))} rows={3} className="w-full bg-[#1a1410] border border-[#2e2016] rounded-lg px-3 py-2 text-sm text-[#c4b49a] focus:outline-none focus:border-[#c8843a]/50 resize-none" />
            </div>
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={() => { if (form.name.trim()) createMutation.mutate(form) }} disabled={!form.name.trim() || createMutation.isPending} className="flex-1 px-4 py-2 bg-[#c8843a] hover:bg-[#b8732a] disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1f1a14] border border-[#2e2016] rounded-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-sm text-[#c4b49a]/70">Delete {confirmDelete.length === 1 ? 'this function' : `${confirmDelete.length} functions`}? This cannot be undone.</p>
