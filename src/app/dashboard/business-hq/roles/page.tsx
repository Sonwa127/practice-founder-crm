...
type RoleStatus = 'Not Filled' | 'Filled' | 'Temporarily Filled'
...
const STATUS_OPTIONS: RoleStatus[] = ['Not Filled', 'Temporarily Filled', 'Filled']
const STATUS_STYLE: Record<RoleStatus, string> = {
  'Not Filled':         'bg-red-900/20 text-red-400 border border-red-700/30',
  'Temporarily Filled': 'bg-amber-900/30 text-amber-300 border border-amber-700/40',
  'Filled':             'bg-green-900/30 text-green-300 border border-green-700/40',
}
...
function RolesContent() {
  ...
  const emptyForm = { name: '', description: '', status: 'Not Filled' as RoleStatus, linked_employee_id: '' }
  ...
  const createMutation = useMutation({
    mutationFn: async (p: typeof emptyForm) => {
      const { error } = await supabase.from('roles').insert({ org_id: orgId, name: p.name, description: p.description || null, status: p.status, linked_employee_id: p.linked_employee_id || null })
      if (error) throw error
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, field, value }: { id: string; field: string; value: string }) => {
      const { error } = await supabase.from('roles').update({ [field]: value || null }).eq('id', id)
      if (error) throw error
    },
  ...
  // Counts by status
  const counts = { filled: items.filter(i => i.status === 'Filled').length, temp: items.filter(i => i.status === 'Temporarily Filled').length, unfilled: items.filter(i => i.status === 'Not Filled').length }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-[#c4b49a]">Roles</h1>
          <p className="text-sm text-[#c4b49a]/60 mt-1">Every role in the practice and who fills it</p>
        </div>
        <button onClick={() => setShowForm(true)} className="px-4 py-2 bg-[#c8843a] hover:bg-[#b8732a] text-white text-sm font-medium rounded-lg transition-colors">+ New Role</button>
      </div>

      {/* Status summary cards */}
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Filled', value: counts.filled, color: 'text-green-400' },
          { label: 'Temp Filled', value: counts.temp, color: 'text-amber-400' },
          { label: 'Not Filled', value: counts.unfilled, color: 'text-red-400' },
        ].map(s => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search roles…" className="w-full pl-9 pr-4 py-2 bg-[#1f1a14] border border-[#2e2016] rounded-lg text-sm text-[#c4b49a] placeholder-[#c4b49a]/30 focus:outline-none focus:border-[#c8843a]/50" />
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value as RoleStatus | 'All')} className="px-3 py-2 bg-[#1f1a14] border border-[#2e2016] rounded-lg text-sm text-[#c4b49a] focus:outline-none focus:border-[#c8843a]/50">
      ...
      <div className="bg-[#1f1a14] border border-[#2e2016] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#2e2016]">
              ...
              <th className="p-3 text-left text-xs uppercase tracking-widest text-[#c4b49a]/50 font-medium">Role Name</th>
              <th className="p-3 text-left text-xs uppercase tracking-widest text-[#c4b49a]/50 font-medium">Status</th>
              <th className="p-3 text-left text-xs uppercase tracking-widest text-[#c4b49a]/50 font-medium">Filled By</th>
              <th className="p-3 text-left text-xs uppercase tracking-widest text-[#c4b49a]/50 font-medium">Description</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="p-12 text-center text-[#c4b49a]/40 text-sm">No roles yet.</td></tr>
            ) : filtered.map(item => (
              <>
                <tr key={item.id} className={`border-b border-[#2e2016]/50 hover:bg-[#2a1f14]/40 transition-colors ${expandedId === item.id ? 'bg-[#2a1f14]/40' : ''}`}>
                  ...
                  <td className="p-3 text-[#c4b49a]/70 text-xs">{item.employees?.name ?? '—'}</td>
                  <td className="p-3 text-[#c4b49a]/60 text-xs max-w-xs truncate">{item.description ?? '—'}</td>
                </tr>
                    ...
                    <td colSpan={6} className="p-0">
                      <div className="p-6 grid grid-cols-2 gap-8">
                        <div className="space-y-5">
                          <div><p className="text-xs text-[#c4b49a]/40 uppercase tracking-widest mb-1">Role Name</p>
                            {editingId === item.id && editField === 'name' ? <input autoFocus value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={commitEdit} onKeyDown={e => { if (e.key === 'Enter') commitEdit() }} className="w-full bg-[#1f1a14] border border-[#c8843a]/50 rounded-lg px-3 py-2 text-sm text-[#c4b49a] focus:outline-none" /> : <p className="text-sm text-[#c4b49a]/80 cursor-pointer hover:text-[#c4b49a]" onClick={() => startEdit(item.id, 'name', item.name)}>{item.name}</p>}
                          </div>
                          <div><p className="text-xs text-[#c4b49a]/40 uppercase tracking-widest mb-1">Description</p>
                            {editingId === item.id && editField === 'description' ? <textarea autoFocus value={editValue} onChange={e => setEditValue(e.target.value)} onBlur={commitEdit} rows={4} className="w-full bg-[#1f1a14] border border-[#c8843a]/50 rounded-lg px-3 py-2 text-sm text-[#c4b49a] focus:outline-none resize-none" /> : <p className="text-sm text-[#c4b49a]/80 cursor-pointer hover:text-[#c4b49a] min-h-[2rem] leading-relaxed" onClick={() => startEdit(item.id, 'description', item.description ?? '')}>{item.description || <span className="text-[#c4b49a]/30 italic">Click to add…</span>}</p>}
                          </div>
                          <div><p className="text-xs text-[#c4b49a]/40 uppercase tracking-widest mb-1">Filled By (Employee)</p>
                            {editingId === item.id && editField === 'linked_employee_id' ? (
                              ...
                              <p className="text-sm text-[#c4b49a] cursor-pointer hover:text-[#c8843a]" onClick={() => startEdit(item.id, 'linked_employee_id', item.linked_employee_id ?? '')}>
                                {item.employees?.name ?? <span className="text-[#c4b49a]/30 italic">None</span>}
                              </p>
                            )}
                          </div>
                        </div>
                        <div><RecordComments orgId={orgId} recordId={item.id} tableName="roles" /></div>
                      </div>
                      <div className="px-6 pb-4 flex justify-end">
                        <button onClick={() => setConfirmDelete([item.id])} className="text-xs text-red-400/60 hover:text-red-400 transition-colors">Delete role</button>
                      </div>
                    </td>
                  </tr>
                )}
        ...
        {filtered.length > 0 && <div className="px-4 py-3 border-t border-[#2e2016] text-xs text-[#c4b49a]/40">Showing {filtered.length} of {items.length} roles</div>}
      </div>

      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1f1a14] border border-[#2e2016] rounded-2xl w-full max-w-lg p-6 space-y-4">
            <div className="flex items-center justify-between"><h2 className="text-lg font-semibold text-[#c4b49a]">New Role</h2><button onClick={() => setShowForm(false)} className="text-[#c4b49a]/40 hover:text-[#c4b49a]"><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12"/></svg></button></div>
            <div><label className="block text-xs text-[#c4b49a]/50 uppercase tracking-widest mb-1">Role Name *</label><input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className="w-full bg-[#1a1410] border border-[#2e2016] rounded-lg px-3 py-2 text-sm text-[#c4b49a] focus:outline-none focus:border-[#c8843a]/50" /></div>
            <div><label className="block text-xs text-[#c4b49a]/50 uppercase tracking-widest mb-1">Status</label><select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value as RoleStatus }))} className="w-full bg-[#1a1410] border border-[#2e2016] rounded-lg px-3 py-2 text-sm text-[#c4b49a] focus:outline-none focus:border-[#c8843a]/50">{STATUS_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}</select></div>
            <div><label className="block text-xs text-[#c4b49a]/50 uppercase tracking-widest mb-1">Filled By</label><select value={form.linked_employee_id} onChange={e => setForm(p => ({ ...p, linked_employee_id: e.target.value }))} className="w-full bg-[#1a1410] border border-[#2e2016] rounded-lg px-3 py-2 text-sm text-[#c4b49a] focus:outline-none focus:border-[#c8843a]/50"><option value="">— None —</option>{employees.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select></div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1f1a14] border border-[#2e2016] rounded-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-sm text-[#c4b49a]/70">Delete {confirmDelete.length === 1 ? 'this role' : `${confirmDelete.length} roles`}? This cannot be undone.</p>
